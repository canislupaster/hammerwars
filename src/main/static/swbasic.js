...
function Parser(text)
{
	...
	this.expand_if = function(dest, ifs, idx, line) {
		...
		var newif = new ASTNode("IF");
		newif.addChild(ifs.children[0]);
		newif.addChild(new ASTNode("GOTO", label_true));
		if (ifs.children.length==3) {
			//only if there is else part
			newif.addChild(new ASTNode("GOTO", label_false));
		} else {
			newif.addChild(new ASTNode("GOTO", label_done));
		}
		dest.push(newif);
		dest.push(new ASTNode("LINENUMBER", label_true));
		...
		//have else part
		if (ifs.children.length==3) {
			dest.push(new ASTNode("GOTO", label_done));
			var elsepart = ifs.children[2];
			dest.push(new ASTNode("LINENUMBER", label_false));
			for (var i = 0; i<elsepart.children.length; i++) {
				dest.push(elsepart.children[i]);
			}
		}
		dest.push(new ASTNode("LINENUMBER", label_done));
	}

	this.flatten = function() {
		for (var i = 0; i<this.lines.length; i++) {
			var line = this.lines[i];
			this.statements.push(new ASTNode("LINENUMBER", line.linenumber));
	...
	this.processLine = function(linenum) {
		l = new Line(linenum);
	...
	this.atom = function() {
		if (this.accept("NUMBER")) {
			return new ASTNode("NUMBER", parseFloat(this.lastText()));
			...
			return new ASTNode("STRING", this.lastText());
	...
	this.unary_expr = function() {
		if (this.acceptText("+")) {
			var node = new ASTNode("UNARY_PLUS");
			node.addChild(this.atom());
			return node;
		}
		if (this.acceptText("-")) {
			var node = new ASTNode("UNARY_MINUS");
			node.addChild(this.atom());
			return node;
		}
		return this.atom();
	...
	this.mult_expr = function() {
		...
		while (this.accept("MULT_OPERATOR")) {
			var opnode = new ASTNode(this.lastText());
	...
	this.input_statement = function(self) {
		var node = new ASTNode("INPUT");

		var q = "?";
		if (self.accept("STRING")) {
			q = self.lastText();
		}
		var query = new ASTNode("STRING", q);
	...
	this.randomize_statement = function(self) {
		var node = new ASTNode("RANDOMIZE");
		...
		if (self.will_accept("NUMBER") || self.will_accept("STRING")) {
			while (self.hasMoreTokens()) {
				if (self.accept("NUMBER")) {
					...
					node.addChild(new ASTNode("DATUM", text));
				} else if (self.accept("STRING")) {
					node.addChild(new ASTNode("DATUM", self.lastText()));
				} else {
	...
	this.next_statement = function(self) {
		var node = new ASTNode("NEXT");
		if (self.accept("IDENTIFIER")) {
			while (self.hasMoreTokens()) {
				node.addChild(new ASTNode("IDENTIFIER", self.lastText()));
				if (!self.acceptText(","))
					break;
				if (!self.accept("IDENTIFIER"))
					break;
	...
	this.wend_statement = function(self) {
		var node = new ASTNode("WEND");
		return node;
	}

	this.clear_statement = function(self) {
		var node = new ASTNode("CLEAR");
		return node;
	}

	this.if_statement = function(self) {
		var node = new ASTNode("IF");
		var expr = self.expression();
		node.addChild(expr);
		...
		if (self.acceptText("THEN")) {
			...
			var thenpart = new ASTNode("THEN");
			var statement = null;

			if (self.will_accept("NUMBER")) {
				self.accept("NUMBER");
				statement = new ASTNode("GOTO");
			...
			if (self.acceptText("ELSE")) {
				var elsepart = new ASTNode("ELSE");

				if (self.will_accept("NUMBER")) {
					self.accept("NUMBER");
					var gstatement = new ASTNode("GOTO");
			...
			var gpart = new ASTNode("GOTO");

			if (self.accept("NUMBER")) {
				gpart.text = self.lastText();
				node.addChild(gpart);
				//Ignore rest of line
				var ignore = new ASTNode("_");
				self.accept("STATEMENT_DELIMITER");
	...
	this.print_statement = function(self) {
		...
		while (self.hasMoreTokens()) {
			...
			if (self.accept("DELIMITER")) {
				node.addChild(new ASTNode("DELIMITER", self.lastText()));
			}
	...
	this.let_statement = function(self) {
		...
		if (self.acceptText("=")) {
			...
			var anode = new ASTNode("ASSIGNMENT");
...
function Interpreter(parser)
{
	...
	this.input_statement = function(self, idx) {
		...
		if (self.print_function && self.last_input_var==0) {
			self.print_function(prompt, true);
